...
class PgNode_a(PgNode):
    ...
    def __init__(self, action: Action):
        ...
        PgNode.__init__(self)
        self.action = action
        self.prenodes = self.precond_s_nodes()  # set
        self.effnodes = self.effect_s_nodes()  # set
...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        self.s_levels = []  # list of sets
        self.a_levels = []  # list of sets
        self.create_graph()
    ...
    def add_action_level(self, level):
        ...
        """
        # Iterate over all actions possible for the problem
        self.a_levels.append(set())

        for action in self.all_actions:

            candidate_node_a = PgNode_a(action)
            # all prerequisite literals for the action hold in s_levels[level]
            has_all_parents = candidate_node_a.prenodes.issubset(self.s_levels[
                                                                 level])
            # if all prereq. satisfied add it to action level 'level'
            if has_all_parents:
                # connect parents
                for parent in candidate_node_a.prenodes:
                    parent.children.add(candidate_node_a)
                    candidate_node_a.parents.add(parent)

                # add child to this level
                self.a_levels[level].add(candidate_node_a)
    ...
    def add_literal_level(self, level):
        ...
        for parent_a_node in self.a_levels[level - 1]:
            for s_node in parent_a_node.effnodes:
                # populate level, duplicate taken care of
                self.s_levels[level].add(s_node)

        # connecting each added node to its parents
        for s_node in self.s_levels[level]:
            for parent_a_node in self.a_levels[level - 1]:
                if s_node in parent_a_node.effnodes:
                    parent_a_node.children.add(s_node)
                    s_node.parents.add(parent_a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        pos1 = set(node_a1.action.effect_add)
        neg1 = set(node_a1.action.effect_rem)

        pos2 = set(node_a2.action.effect_add)
        neg2 = set(node_a2.action.effect_rem)

        return bool(pos1 & neg2) or bool(neg1 & pos2)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool(pos1 & pre_neg2) or bool(neg1 & pre_pos2) or bool(pos2 & pre_neg1) or bool(neg2 & pre_pos1)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for precond_s_node1 in node_a1.parents:
            for precond_s_node2 in node_a2.parents:
                if precond_s_node1.is_mutex(precond_s_node2) or precond_s_node2.is_mutex(precond_s_node1):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a1 in node_s1.parents:
            for a2 in node_s2.parents:
                # if an action pair is not mutex return False as literal are
                # not mutex now
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            ...
            for level, states in enumerate(self.s_levels):
                if goal in states:
                    level_sum += level
                    break  # only first appearance of goal
